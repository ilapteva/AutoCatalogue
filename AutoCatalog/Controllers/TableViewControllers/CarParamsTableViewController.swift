...
//        private let reuseIdentifier = "paramsCell"
    ...
    @IBAction func saveButtonPressed(_ sender: UIBarButtonItem) {
            ...
            if car == nil {
                var maxId = realm.objects(ServiceRealm.self).map{$0.id}.max() ?? 0
                maxId = maxId + 1
        
                
                let newCar = CarParams(id: maxId, brand: brand, year: releaseYear, model: model,body: carBody)
                    ServiceRealm.save(autoObject: newCar)
                
                navigationController?.popViewController(animated: true)
                
            } else {
                guard let updatedCar = realm.objects(ServiceRealm.self).filter("id == \(car!.id)").first else { return }
                try! realm.write {
    ...
    extension CarParamsTableViewController {
        ...
        override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
            let cell = tableView.dequeueReusableCell(withIdentifier: "carParamsCell", for: indexPath) as! CarParamsTableViewCell
